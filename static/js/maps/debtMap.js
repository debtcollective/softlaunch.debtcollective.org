  ...
  function($scope, $http, util_svc) {
    ...
    ************/
    ...
    $http.get('/static/js/map_data.json').then(function(resp) {
      ...
      for (idx in points) {
        ...
        point['scale'] = getPointScale(
          parseFloat(point['sum_amount'] / total_amount),
        );
    ...
    ***********/
    ...
    map.addListener('clickMapObject', function(event) {
      console.log(event);
      if (
        event.mapObject.id != undefined &&
        chartData[event.mapObject.id] != undefined
      ) {
